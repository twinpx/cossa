...
$(document).ready(function() {
	...
	$("#comments_block div.comment").hover(function() {$(this).find("span.comment_menu").find("a.link, a.edit, a.delete, a.reply").css({display: "inline-block"});
...
function downloadLikeButtons() {
	...
	var vars = {};
	vars.networks = ["facebook", "vkontakte", "twitter"];
	...
	function handleEvents() {
		...
		function lineTwitterOver() {
			var $likeIcon = $(this);
			var $data = $likeIcon.closest(".like_buttons").find(".like_buttons_data");
			$likeIcon.before('<iframe allowtransparency="true" frameborder="0" scrolling="no" src="https://platform.twitter.com/widgets/tweet_button.html?count=horizontal&counturl=' + $data.attr("data-url") + '&text=' + $data.attr("data-text") + '" style="width:100px; height:20px;"></iframe>');
			$likeIcon.remove();
		}
		...
		function sideTwitterOver() {
			var $likeIcon = $(this);
			var $data = $likeIcon.closest(".like_buttons").find(".like_buttons_data");
			$likeIcon.before('<iframe allowtransparency="true" frameborder="0" scrolling="no" src="https://platform.twitter.com/widgets/tweet_button.html?count=vertical&counturl=' + $data.attr("data-url") + '&text=' + $data.attr("data-text") + '" style="width:58px; height:62px;"></iframe>');
			$likeIcon.remove();
		}
	}
	...
	function sendAjaxReq() {
		$.ajax({
			url: "http://www.cossa.ru/like/likes.php",
			dataType: "json",
			data: "networks=" + vars.networks.join() + "&id=" + vars.articlesId.join() + "&secId=" + vars.sectionId.join(),
			success: function(data) {
				createLikes(data);
			},
			error: ajaxError
	...
	function createIdArray() {
		...
		else if(document.getElementById("articles_list")) {
